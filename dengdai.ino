...
unsigned char pid[] = { "7oinlc8lpvhcdnnu" }; //*********处替换成涂鸦IoT平台自己创建的产品的PID
unsigned char mcu_ver[] = { "1.0.0" };

unsigned long last_time = 0;
SoftwareSerial DebugSerial(A3, A4); //rx tx
void setup()
{
...
unsigned char dp_process(unsigned char dpid, const unsigned char value[], unsigned short length)
{
    ...
    case DPID_SWITCH_LED:
        ...
        // DebugSerial.print(DPID_SWITCH_LED);
        // DebugSerial.print("   ");
        DebugSerial.println(dp_bool_value);
        if (dp_bool_value) {
            //Turn on
    ...
    case DPID_DREAMLIGHT_SCENE_MODE: //炫彩情景
        ...
        break;
    case DPID_DREAMLIGHTMIC_MUSIC_DATA: //炫彩本地音乐律动
    {
        my_device.mcu_dp_update(DPID_DREAMLIGHTMIC_MUSIC_DATA, value, length);
        //DebugSerial.print(" ");
        //DebugSerial.print(value);
        // for (int i = 0; i <= length; i++) {
        //     scene_mode = value[i];
        //     DebugSerial.print(scene_mode);
        // }
        // dp_value_value = analogRead(A7);
        // DebugSerial.println("A7");
        // DebugSerial.println(dp_value_value);
        deng_dai = 1;
        // for (int i = 0; i <= PIXEL_COUNT; i++) {
        //     strip.setPixelColor(i, strip.ColorHSV(analogRead(A7) * 127));
        // }
...
void local_music_fuc(void)
{
    for (int i = 0; i <= PIXEL_COUNT; i++)
        strip.setPixelColor(i, strip.ColorHSV(analogRead(A7) * 255));
    strip.show();
    delay((1024-analogRead(A7))/10);
}
